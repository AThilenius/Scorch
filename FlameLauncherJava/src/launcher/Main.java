...
public class Main {
    ...
    private static String globLibFolder() {
        String globAll = globFiles(new File("libraries/").listFiles(), ":");
        globAll = globAll.substring(0, globAll.length() - 1);
        globAll.replace(" ", "\\ ");
        return globAll;
    }

    private static String globFiles(File[] files, String separator) {
        String glob = "";
        for (File file : files) {
            if (file.isDirectory()) {
                glob += globFiles(file.listFiles(), separator);
            } else {
    ...
    public static void main(String[] args) {
        ...
        String minecraftPath = new File("").getAbsolutePath();
        //String minecraftPath = "/Users/Alec/Documents/Development/Scorch/FlameLauncherJava";
        ...
        System.out.println("PWD Path: " + launcher.Main.class.getProtectionDomain().getCodeSource().getLocation().getPath());
        ...
        if (proc != null) {
            ...
            out.println("libs_glob=" + globLibFolder());
